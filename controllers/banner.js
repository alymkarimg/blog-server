...
const {
  uploadBannerImages,
  uploadBannerImageURLs,
} = require("../helpers/imageUploader");

exports.loadAnimatedBanner = async function (
  animatedBannerTitle
) {
  var animatedBanner = await AnimatedBanner.findOne({
    title: animatedBannerTitle,
  });

  // if there is no editable area, create one
  if (!animatedBanner) {
    animatedBanner = AnimatedBanner.createBanner(animatedBannerTitle);
  }

  await animatedBanner.populate("items").execPopulate();
  await Promise.all(
    animatedBanner.items.map(async (bannerItem) => {
...
exports.loadAllAnimatedBanners = async function (req, res, next) {
  // for each animated banner
  let banners = await Promise.all(
    req.body.animatedBanners.map(async (animatedBanner) => {
      let bannerModel = exports.loadAnimatedBanner(animatedBanner.title);
      return bannerModel;
    })
  );
...
exports.addSlides = async function (req, res, next) {
  ...
  res.json({
    animatedBanner,
    message: "Slide added",
    length: animatedBanner.items.length,
  });
};

exports.deleteSlide = async function (req, res) {
  try {
    var banner = await AnimatedBanner.findOne({
      title: req.body.animatedBanner.title,
    });
    if (banner) {
      if (banner.items.length == 1) {
        ...
        banner.items = await Promise.all(
          banner.items.map(async (bannerItem) => {
            await bannerItem.populate("EditableArea").execPopulate();
            return bannerItem;
          })
        );
        ...
        banner.items = banner.items.filter(
          (q) => q.guid != req.body.slideNumber
        );

        await AnimatedBannerItem.deleteOne({
          pathname: banner.title,
          guid: req.body.slideNumber,
        });
        await EditableArea.deleteOne({
          pathname: banner.title,
          guid: req.body.slideNumber,
        });
        ...
        async function array_values(arrayObj) {
          ...
          await Promise.all(
            arrayObj.map(async (item, index) => {
              var area = await EditableArea.findOne({
                guid: item.guid,
